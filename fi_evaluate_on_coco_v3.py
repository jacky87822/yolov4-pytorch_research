...
from tool.torch_utils import fi_do_detect,post_inj,post_inj_half
...
def test(model, annotations, cfg, inj_list, path_name):
    ...
    pfi_model = fault_injection(model, 
        ...
        layer_types=[torch.nn.BatchNorm2d,torch.nn.Conv2d],
        use_cuda=True,
        )
    bn_layer=[]
    for lc,name in enumerate (pfi_model.layers_type):
        if "Norm" in str(name):
            bn_layer.append(lc)
    ...
    ef_list,wo_list=[],[]
    f=open(path_name+"/inj_list(p).txt",'w')
    for i in inj_list:
        #print (i)
        layer.append(i[0])
        k.append(i[1])
        ...
        ########################################################################
        # protection method
        inject_value=protect_method.protect_method(i,cfg,bn_layer)
        
        ########################################################################
        err_val.append(inject_value)
        ef_list.append(i[6][0])
        wo_list.append(i[6][cfg.inj_mode])
        line=i[0], i[1], i[2], i[3], i[4], [i[6][0],inject_value]
        f.write(str(line)+'\n')
    f.close
    logging.info('finish write inject txt')
    
    inj_info=protect_method.MSE(ef_list,wo_list,err_val,num[1],device)
    logging.info('finish evaluate inject value')
    ...
    inj = pfi_model.get_corrupted_model()


    if cfg.half:
        boxes = post_inj_half(inj, sized, 0.001,  0.6, use_cuda)
    else:
        boxes = post_inj(inj, sized, 0.001,  0.6, use_cuda)
...
########################################################################################
    ...
    summary_list=[datetime.now(),cfg.tmode,path_name,cfg.weights_file,num[0],num[1],
                  in_ber,len(inj_list),seed,check,cfg.inj_mode,cfg.protect,
                  count_one_fault,count_multi_fault,
                  t_op,failure_box,failure_img]
    print (inj_info)
    summary_list.extend(inj_info)
    print (summary_list)
    ...
    summary+=("check     : "+str(summary_list[9])+'\n')
    summary+=("INJ mode  : "+str(summary_list[10])+'\n')
    summary+=("PRT mode  : "+str(summary_list[11])+'\n')
    summary+=("#s-fault  : "+str(summary_list[12])+'\n')
    summary+=("#m-fault  : "+str(summary_list[13])+'\n')

    summary+=("\n") 
    summary+=("dur_time  : "+str(summary_list[14])+'\n')
    summary+=("#fail img : "+str(summary_list[15])+'\n')
    summary+=("#fail img : "+str(summary_list[16])+'\n')
    
    summary+=("\n") 
    summary+=("MSE_wo    : "+str(summary_list[17])+'\n')
    summary+=("MSE_wp    : "+str(summary_list[18])+'\n')
    summary+=("max value : "+str(summary_list[19])+'\n')
    summary+=("min value : "+str(summary_list[20])+'\n')
    summary+=("======================================================================="+'\n')
    ...
    try:
        ...
        mAP_result=True
        r=[Ap_result[1],Ap_result[-3],Ap_result[-2],Ap_result[-1]]
...
if __name__ == "__main__":
    ...
    header=["Time","Testmode","Result","Weights","#layers","#weights", #5
        "BER","#inject","seed","check","INJ mode","PRT mode","#s-fault","#m-fault",
        "dur_time","#fail box","#fail img",
        "MSE wo","MSE wp","max","min",
    ...
    for iter,curr_ber in enumerate (ber_list):
        ...
        for s_iter, curr_seed in enumerate (seed_list):
            ...
            if s_exist==False:
                ...
                num_inj_list=len(inj_list)
                
                logging.info("write out inject list ...")
                f=open(path_name+"/inj_list.txt",'w')
                for i in inj_list:
                    f.write(str(i)+'\n')
                f.close
        ...
        if org_ber==-1:
            ...
            if org_ber==-1 and iter==len(ber_list)-1:
                ...
                if (pre_map==-1):
                    ...
                    ber_list.extend([float(format(curr_ber*10,'.8f'))])
                    ...
                    if curr_map<0.001:
                        ...
                        ber_list.extend([float(format(curr_ber*0.2, '.8f')), 
                                    float(format(curr_ber*0.4, '.8f')), 
                                    float(format(curr_ber*0.6, '.8f')), 
                                    float(format(curr_ber*0.8, '.8f')),
                                    float(format(-1,'.8f'))]) 
                        ...
                        ber_list.extend([float(format(curr_ber*0.2, '.8f')), 
                                    ...
                                    float(format(curr_ber*0.6, '.8f')), 
                                    float(format(curr_ber*0.8, '.8f')),
                                    float(format(curr_ber*2, '.8f')), 
                                    #float(format(curr_ber*4, '.8f')), 
                                    #float(format(curr_ber*6, '.8f')), 
                                    float(format(-1, '.8f'))])   
                    else:
                        logging.info("......curr_map>=0.001:")
                        ber_list.extend([float(format(curr_ber*0.2, '.8f')), 
