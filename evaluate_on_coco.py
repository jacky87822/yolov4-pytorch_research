...
from tool.torch_utils import do_detect, fi_do_detect, do_detect_half
import cv2
...
def test(model, annotations, cfg):
    ...
    logging.info("image input shape: {}/{}".format(model.width, model.height))
    ...
    Ap_result=evaluate_on_coco(cfg, resFile)
    return Ap_result
...
def get_args(**kwargs):
    ...
    parser.add_argument('-w', '--weights_file', type=str, default='../model/yolov4.weights',
                        help='weights file to load', dest='weights_file')
    parser.add_argument('-c', '--model_config', type=str, default='../model/yolov4.cfg',
...
if __name__ == "__main__":
    ...
    Ap_result=test(model=model,
         annotations=annotations,
         cfg=cfg)
    
    if not os.path.exists("eva_result"):
        os.makedirs("eva_result")    
    ...
    summary=""
    w_name=cfg.weights_file.split('/')[-1]
    w_name=w_name.split('.')[0]
    print (w_name)
    fn=open("eva_result/"+w_name+'.txt','w')
