...
def test(model, annotations, cfg, inj_list, path_name):
    ...
    for i in inj_list:
        ...
        '''
        if cfg.protect==0: # no protect
            inject_value=i[6][cfg.inj_mode]
        elif cfg.protect==1: # zero-masking
            inject_value=0
        elif cfg.protect==2:
            inject_value=i[6][0]
        elif cfg.protect==3 and abs(i[6][cfg.inj_mode])>4096:
            if i[6][cfg.inj_mode]<0:
                inject_value=-4096
            else:
                inject_value=+4096
...
########################################################################################
    ...
    summary+=("#layers   : "+str(num[0])+'\n')
    summary+=("#weights  : "+str(num[1])+'\n')

    summary+=("\n") 
    summary+=("BER       : "+str(in_ber)+'\n')
    summary+=("#inject   : "+str(len(inj_list))+'\n')
    summary+=("seed      : "+str(seed)+'\n')
    summary+=("check     : "+str(check)+'\n')
    summary+=("INJ mode  : "+str(cfg.inj_mode)+'\n')
    summary+=("PRT mode  : "+str(cfg.protect)+'\n')
    summary+=("#s-fault  : "+str(count_one_fault)+'\n')
    summary+=("#m-fault  : "+str(count_multi_fault)+'\n')
...
def get_args(**kwargs):
    ...
    parser.add_argument('-w', '--weights_file', type=str, 
                        default='../compress_model/yolov4-tiny_np02.pt',
                        help='weights file to load', dest='weights_file')
    parser.add_argument('-c', '--cfg_file', type=str, 
                        default='../compress_model/normal_prune_0.2_yolov4-tiny.cfg',
                        help='cfg file to load', dest='cfg_file')
    #---------------------------------------------------------------------------------------
    ...
    for iter,curr_ber in enumerate (ber_list):
        ...
        for s_iter, curr_seed in enumerate (seed_list):
            ...
            if s_exist==False:
                ...
                model = Darknet(cfg.cfg_file).to(device)
                model.load_state_dict(torch.load(cfg.weights_file, map_location=device))   ###
