        ...
        for layer, (name, para) in enumerate(model.named_parameters()): 
            ...
            if len(para_shape) == 4:
                    ...
                    for c in range (para_shape[1]):
                        for h in range (para_shape[2]):
                            for w in range (para_shape[3]):
                                ...
                                value=str(binary(model.state_dict()[name][k,c,h,w]))
                                value_split=[*value]
                                #print ('value',value)
                                new_value_list=value_split[:16]+value_split[1:9]+value_split[1:9]
                                new_value = ''.join(new_value_list)
                                #print ('new_va',new_value)
                                model.state_dict()[name][k,c,h,w]=ieee_754_conversion(int(new_value,2))
                ...
                for i in range(para_shape[0]):
                    ...
                    new_value_list=value_split[:16]+value_split[1:9]+value_split[1:9]
                    new_value = ''.join(new_value_list)
                    model.state_dict()[name][i]=ieee_754_conversion(int(new_value,2))
            else:
                print (para_shape)
                exit()
